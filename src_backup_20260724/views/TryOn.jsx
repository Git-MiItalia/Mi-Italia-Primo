...
function ProcessingModal({ open, onCancel, isBatch }) {
  ...
  return (
    <div style={{position:'fixed',inset:0,background:'rgba(26,18,9,0.7)',zIndex:300,display:'flex',alignItems:'center',justifyContent:'center'}}>
      <div style={{background:'var(--white)',borderRadius:20,width:380,overflow:'hidden',boxShadow:'0 20px 60px rgba(26,18,9,0.3)'}}>
        ...
        <div style={{padding:'24px 28px'}}>
          ...
          <div style={{height:5,background:'var(--mist)',borderRadius:3,overflow:'hidden',marginBottom:8}}>
            <div style={{height:'100%',background:'linear-gradient(90deg,var(--gold),var(--gold-light))',borderRadius:3,width:`${progress}%`,transition:'width 0.3s ease'}} />
          </div>
    ...
    <div style={{position:'fixed',inset:0,zIndex:400,display:'flex',justifyContent:'flex-end'}} onClick={onClose}>
      <div style={{width:420,background:'var(--white)',height:'100vh',display:'flex',flexDirection:'column',boxShadow:'-8px 0 40px rgba(26,18,9,0.12)',overflowY:'auto'}} onClick={e=>e.stopPropagation()}>
        ...
        <div className="sheet-body">
          {/* Large image */}
          <div style={{width:'100%',aspectRatio:'3/4',background:"url('https://images.unsplash.com/photo-1539109136881-3be0616acf4b?w=600&h=800&fit=crop&q=80') center/cover",borderRadius:14,marginBottom:20}} />
          {/* Tools */}
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:18}}>
            {tools.map((t,i) => (
              <div key={i} onClick={() => setActiveToolIdx(i)}
                style={{padding:'14px',background:activeToolIdx===i?'rgba(184,149,90,0.08)':'var(--cream)',border:`1.5px solid ${activeToolIdx===i?'var(--gold)':'var(--mist)'}`,borderRadius:12,cursor:'pointer',transition:'all 0.15s'}}>
                <div style={{width:32,height:32,borderRadius:9,background:activeToolIdx===i?'rgba(184,149,90,0.15)':'var(--white)',display:'flex',alignItems:'center',justifyContent:'center',marginBottom:8}}>
          ...
          {/* Regenerate note */}
          <div style={{padding:'13px 14px',background:'rgba(184,149,90,0.06)',borderRadius:11,fontSize:11,lineHeight:1.5,color:'var(--deep)'}}>
  ...
  return (
    <div style={{position:'fixed',inset:0,zIndex:400,display:'flex',justifyContent:'flex-end'}} onClick={onClose}>
      <div style={{width:420,background:'var(--white)',height:'100vh',display:'flex',flexDirection:'column',boxShadow:'-8px 0 40px rgba(26,18,9,0.12)',overflowY:'auto'}} onClick={e=>e.stopPropagation()}>
        ...
        <div className="sheet-body">
          <div style={{background:'rgba(184,149,90,0.08)',border:'1px solid rgba(184,149,90,0.25)',borderRadius:11,padding:'13px 15px',fontSize:11,lineHeight:1.6,marginBottom:18,display:'flex',gap:10,alignItems:'flex-start'}}>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:8,marginBottom:18}}>
            {[
              ...
              <div key={c.name} style={{display:'flex',alignItems:'center',gap:12,padding:'12px 14px',background:'var(--cream)',borderRadius:11,opacity:0.7}}>
                ...
                <div style={{fontSize:7,fontWeight:700,letterSpacing:'0.8px',background:'var(--mist)',color:'var(--stone)',padding:'2px 6px',borderRadius:5}}>COMING SOON</div>
              </div>
            ))}
          </div>
          <div className="sheet-section-label" style={{marginBottom:8}}>Caption <em>preview</em></div>
          <textarea readOnly style={{width:'100%',padding:'10px 12px',background:'var(--cream)',border:'1px solid var(--mist)',borderRadius:10,fontSize:11,fontFamily:'inherit',lineHeight:1.5,resize:'none',opacity:0.7,marginBottom:10}} rows={4} defaultValue="Silk midi in Bordeaux. SS26 collection · made in Italy. Sizes 38–46 available in store and at miitalia.com/atelier-bianchi" />
          <input readOnly style={{width:'100%',padding:'10px 12px',background:'var(--cream)',border:'1px solid var(--mist)',borderRadius:10,fontSize:11,fontFamily:'inherit',opacity:0.7}} defaultValue="#atelierbianchi #miitalia #silkdress #ss26 #madeinitaly" />
...
function ResultsModal({ open, onClose, variants, selectedAspect }) {
  ...
  return (
    <div style={{position:'fixed',inset:0,background:'rgba(26,18,9,0.7)',zIndex:300,display:'flex',alignItems:'center',justifyContent:'center',padding:24,overflowY:'auto'}}>
      <div style={{background:'var(--white)',borderRadius:20,width:'100%',maxWidth:900,boxShadow:'0 20px 60px rgba(26,18,9,0.3)'}}>
        {/* Head */}
        <div style={{padding:'24px 28px',borderBottom:'1px solid var(--mist)',display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:14}}>
          ...
          <div style={{width:32,height:32,borderRadius:9,background:'var(--cream)',display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',flexShrink:0}} onClick={onClose}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:16,marginBottom:28}}>
            {results.map((r,i) => (
              <div key={i} onClick={() => toggleCard(i)}
                style={{borderRadius:12,border:`2px solid ${selectedCards.includes(i)?'var(--gold)':'var(--mist)'}`,overflow:'hidden',cursor:'pointer',background:'var(--white)',transition:'all 0.15s',opacity:fading[i]?0.4:1}}>
                <div style={{background:`url('${r.img}') center/cover`,aspectRatio:'3/4',position:'relative'}}>
                  <div style={{position:'absolute',bottom:8,left:8,background:'rgba(255,255,255,0.95)',fontSize:9,fontWeight:700,padding:'3px 7px',borderRadius:6,color:r.conf>=85?'var(--green)':'var(--gold)',display:'flex',alignItems:'center',gap:3}}>
                    <span className="material-symbols-outlined" style={{fontSize:11}}>{r.conf>=85?'check_circle':'info'}</span>{r.conf}%
                  </div>
                  <div style={{position:'absolute',top:8,right:8,width:22,height:22,borderRadius:6,background:'rgba(255,255,255,0.95)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:11,fontWeight:700,color:'var(--deep)'}}>{i+1}</div>
                </div>
                <div style={{padding:'11px 13px'}}>
                  ...
                  <div style={{display:'flex',gap:6}}>
                    <div style={{flex:1,padding:'5px',background:'var(--cream)',borderRadius:7,fontSize:9.5,fontWeight:600,textAlign:'center',cursor:'pointer'}}
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:12}}>
              {[
                ...
                <div key={i} onClick={a.onClick}
                  style={{position:'relative',padding:'16px 14px',background:a.primary?'var(--deep)':'var(--cream)',border:`1.5px solid ${a.primary?'var(--deep)':'var(--mist)'}`,borderRadius:12,cursor:'pointer',transition:'all 0.15s'}}>
                  {a.comingSoon && <div style={{position:'absolute',top:8,right:8,fontSize:7,fontWeight:700,letterSpacing:'0.8px',background:'var(--mist)',color:'var(--stone)',padding:'2px 5px',borderRadius:5}}>COMING SOON</div>}
                  <div style={{width:36,height:36,borderRadius:10,background:a.primary?'rgba(184,149,90,0.2)':'var(--white)',display:'flex',alignItems:'center',justifyContent:'center',marginBottom:10}}>
              ...
              <div className="sheet-section">
                ...
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
                  {[
                    ...
                    <div key={i}>
                      <div style={{aspectRatio:'3/4',background:`url('${c.img}') center/cover`,borderRadius:10}} />
        ...
        <div style={{display:'flex',flexDirection:'column',gap:9}}>
          {MODELS.map(m => (
            <div key={m.id} onClick={() => {setSelectedModel(m.id);setShowModelSheet(false)}}
              style={{display:'flex',alignItems:'center',gap:12,padding:'12px 14px',background:selectedModel===m.id?'rgba(184,149,90,0.05)':'var(--cream)',borderRadius:11,cursor:'pointer',border:`1.5px solid ${selectedModel===m.id?'var(--gold)':'transparent'}`}}>
              <div style={{width:42,height:42,borderRadius:11,flexShrink:0,background:'linear-gradient(135deg,#D4AF72,#8A6A30)',display:'flex',alignItems:'center',justifyContent:'center',color:'white',fontSize:14,fontWeight:700,fontFamily:"'Cormorant Garamond',serif"}}>{m.init}</div>
              <div style={{flex:1}}>
                <div style={{fontSize:12,fontWeight:700,display:'flex',alignItems:'center',gap:7,marginBottom:2}}>
                  {m.name}{m.tag&&<span style={{fontSize:'7.5px',fontWeight:700,background:'var(--gold)',color:'var(--deep)',padding:'1px 5px',borderRadius:5}}>{m.tag}</span>}
                </div>
          ...
          <div style={{display:'flex',alignItems:'center',justifyContent:'center',gap:7,padding:14,border:'1.5px dashed var(--mist)',borderRadius:11,color:'var(--stone)',fontSize:11,fontWeight:600,cursor:'pointer'}}>
    ...
    <div>
        ...
        <div className="batch-side">
          ...
          <div className="batch-side-row" style={{cursor:'pointer'}} onClick={() => { setDraftLook(selectedLook); setShowLookSheet(true) }}>
            ...
            <div className="batch-side-row-value">
              <div style={{width:18,height:18,borderRadius:5,background:`url('${LOOKS.find(l=>l.id===selectedLook)?.img}') center/cover`,flexShrink:0}} />
          ...
          <div className="batch-side-row" style={{cursor:'pointer'}} onClick={() => { setDraftModel(selectedModel); setShowModelSheet(true) }}>
            ...
            <div className="batch-side-row-value">
              <div style={{width:22,height:22,borderRadius:6,background:'linear-gradient(135deg,#D4AF72,#8A6A30)',display:'flex',alignItems:'center',justifyContent:'center',color:'white',fontSize:9,fontWeight:700,flexShrink:0}}>{currentModel?.init}</div>
      ...
      <Sheet open={showLookSheet} onClose={() => setShowLookSheet(false)}
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
            {LOOKS.map(l => (
              <div key={l.id} onClick={() => setDraftLook(l.id)}
                style={{borderRadius:12,border:`1.5px solid ${draftLook===l.id?'var(--gold)':'var(--mist)'}`,background:'var(--white)',cursor:'pointer',overflow:'hidden',transition:'all 0.15s'}}>
                ...
                <div style={{padding:'10px 12px'}}>
                  <div style={{fontSize:12,fontWeight:700,marginBottom:3,display:'flex',alignItems:'center',gap:6}}>
                    {l.label}
                    {l.id==='editorial' && <span style={{fontSize:'7px',fontWeight:700,background:'var(--gold)',color:'var(--deep)',padding:'1px 5px',borderRadius:4,letterSpacing:'0.4px'}}>DEFAULT</span>}
                  </div>
        ...
        <div className="sheet-section">
          ...
          <div style={{padding:24,border:'1.5px dashed var(--mist)',borderRadius:12,textAlign:'center',cursor:'pointer',background:'var(--cream)'}}>
      ...
      <Sheet open={showModelSheet} onClose={() => setShowModelSheet(false)}
        ...
        {(() => {
          ...
          return (
            <div style={{display:'flex',gap:14,padding:'0 0 18px',borderBottom:'1px solid var(--mist)',marginBottom:18}}>
              <div style={{position:'relative',width:120,height:150,borderRadius:12,background:`url('${HERO_PHOTOS[draftModel]}') center/cover`,flexShrink:0,overflow:'hidden'}}>
                <div style={{position:'absolute',top:8,left:8,fontSize:7,fontWeight:700,letterSpacing:'0.8px',background:'rgba(26,18,9,0.75)',color:'var(--cream)',padding:'3px 6px',borderRadius:5}}>REPRESENTATIVE PREVIEW</div>
              </div>
                ...
                <div>
                  ...
                  <div style={{display:'flex',flexWrap:'wrap',gap:5}}>
                    {m.traits.split(' · ').map(t => (
                      <span key={t} style={{padding:'3px 9px',background:'var(--cream)',border:'1px solid var(--mist)',borderRadius:20,fontSize:10,fontWeight:600}}>{t}</span>
                    ))}
                  </div>
                </div>
        ...
        <div style={{display:'flex',alignItems:'center',gap:10,padding:'11px 14px',background:'var(--cream)',borderRadius:10,marginBottom:18,cursor:'pointer'}}>
        ...
        <div className="sheet-section">
          ...
          <div style={{display:'flex',flexWrap:'wrap',gap:6,marginBottom:6}}>
            {['Pale','Light','Mediterranean','Olive','Brown','Deep brown','Dark'].map(v => (
              <div key={v} style={{padding:'5px 11px',borderRadius:20,border:`1.5px solid ${v==='Mediterranean'?'var(--gold)':'var(--mist)'}`,background:v==='Mediterranean'?'rgba(184,149,90,0.08)':'var(--white)',fontSize:10.5,fontWeight:600,cursor:'pointer'}}>{v}</div>
            ))}
          </div>
        ...
        <div className="sheet-section">
          ...
          <div style={{display:'flex',flexWrap:'wrap',gap:6}}>
            {['18-22','22-28','28-32','32-40','40-48','48-60','60+'].map(v => (
              <div key={v} style={{padding:'5px 11px',borderRadius:20,border:`1.5px solid ${v==='28-32'?'var(--gold)':'var(--mist)'}`,background:v==='28-32'?'rgba(184,149,90,0.08)':'var(--white)',fontSize:10.5,fontWeight:600,cursor:'pointer'}}>{v}</div>
            ))}
          </div>
        </div>
        ...
        <div className="sheet-section">
          ...
          <div style={{display:'flex',flexWrap:'wrap',gap:6,marginBottom:6}}>
            {['Petite','Slim','Athletic','Mid-size','Curve','Plus','Tall'].map(v => (
              <div key={v} style={{padding:'5px 11px',borderRadius:20,border:`1.5px solid ${v==='Athletic'?'var(--gold)':'var(--mist)'}`,background:v==='Athletic'?'rgba(184,149,90,0.08)':'var(--white)',fontSize:10.5,fontWeight:600,cursor:'pointer'}}>{v}</div>
            ))}
          </div>
        ...
        <div className="sheet-section">
          ...
          <div style={{display:'flex',flexWrap:'wrap',gap:6}}>
            {['Dark · long','Dark · short','Brown · medium','Blonde · long','Red · medium','Grey · bob','Black · curly','Natural texture · short'].map(v => (
              <div key={v} style={{padding:'5px 11px',borderRadius:20,border:`1.5px solid ${v==='Dark · long'?'var(--gold)':'var(--mist)'}`,background:v==='Dark · long'?'rgba(184,149,90,0.08)':'var(--white)',fontSize:10.5,fontWeight:600,cursor:'pointer'}}>{v}</div>
            ))}
          </div>
        </div>
        ...
        <div className="sheet-section">
          ...
          <div style={{display:'flex',flexWrap:'wrap',gap:6}}>
            {['Relaxed','Editorial · static','Confident stride','Candid · in motion','Quiet · contemplative'].map(v => (
              <div key={v} style={{padding:'5px 11px',borderRadius:20,border:`1.5px solid ${v==='Editorial · static'?'var(--gold)':'var(--mist)'}`,background:v==='Editorial · static'?'rgba(184,149,90,0.08)':'var(--white)',fontSize:10.5,fontWeight:600,cursor:'pointer'}}>{v}</div>
            ))}
          </div>
        </div>

        <div style={{display:'flex',alignItems:'center',gap:10,padding:'12px 14px',background:'var(--cream)',borderRadius:12,marginTop:8,border:'1px solid var(--mist)'}}>
          ...
          <button style={{padding:'7px 14px',background:'var(--gold)',color:'var(--deep)',border:'none',borderRadius:8,fontSize:11,fontWeight:700,cursor:'pointer',flexShrink:0,whiteSpace:'nowrap'}}>Save as preset</button>
        </div>
      </Sheet>
        ...
        <div className="sheet-section" style={{display:'flex',flexDirection:'column',gap:9}}>
          {ASPECT_OPTS.map(o => (
            <div key={o.val} onClick={() => setDraftAspect(o.val)}
              style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'12px 14px',borderRadius:11,cursor:'pointer',border:`1.5px solid ${draftAspect===o.val?'var(--gold)':'transparent'}`,background:draftAspect===o.val?'rgba(184,149,90,0.1)':'var(--cream)'}}>
              <div>
        ...
        <div className="sheet-section" style={{display:'flex',flexDirection:'column',gap:9}}>
          {PARALLEL_OPTS.map(o => (
            <div key={o.val} onClick={() => setDraftParallel(o.val)}
              style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'12px 14px',borderRadius:11,cursor:'pointer',border:`1.5px solid ${draftParallel===o.val?'var(--gold)':'transparent'}`,background:draftParallel===o.val?'rgba(184,149,90,0.1)':'var(--cream)'}}>
              <div>
      ...
      <Sheet open={showVary} onClose={() => setShowVary(false)}
        ...
        <div className="sheet-section">
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'12px 14px',background:'var(--cream)',borderRadius:11,cursor:'pointer',marginBottom:18}} onClick={() => setVaryOn(v => !v)}>
            <div>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:9}}>
            {[
              ...
              <div key={m.init} style={{display:'flex',alignItems:'center',gap:12,padding:'12px 14px',background:'var(--cream)',borderRadius:11}}>
                <div style={{width:36,height:36,borderRadius:10,background:m.bg,display:'flex',alignItems:'center',justifyContent:'center',color:m.color,fontSize:13,fontWeight:700,flexShrink:0}}>{m.init}</div>
                <div style={{flex:1}}>
      ...
      {rowSheet && (
        <Sheet open={!!rowSheet} onClose={() => setRowSheet(null)}
          ...
          <div className="sheet-section">
            ...
            <div style={{padding:'12px 14px',background:'var(--cream)',borderRadius:11,fontSize:10.5,lineHeight:1.55}}>
          ...
          <div className="sheet-section">
            ...
            <div style={{display:'flex',flexWrap:'wrap',gap:6}}>
              {['Use shared','Natural light','Lifestyle Milano','Studio clean'].map(v => (
                <div key={v} style={{padding:'6px 12px',background:'var(--cream)',borderRadius:8,fontSize:10.5,fontWeight:600,cursor:'pointer',border:'1.5px solid var(--mist)'}}>{v}</div>
              ))}
            </div>
          </div>
          <div className="sheet-section">
          ...
          <div className="sheet-section">
            ...
            <div style={{display:'flex',flexDirection:'column',gap:8}}>
              {[
                ...
                <div key={a.label} style={{display:'flex',alignItems:'center',gap:12,padding:'12px 14px',background:'var(--cream)',borderRadius:11,cursor:'pointer'}} onClick={() => a.label==='Open in single mode'&&onNavigate('generate')}>
                  <div style={{flex:1}}>
      ...
      <Sheet open={showAddProducts} onClose={() => setShowAddProducts(false)}
        ...
        <div className="sheet-section">
          ...
          <div style={{display:'flex',flexDirection:'column',gap:8}}>
            {[
              ...
              <div key={a.label} style={{display:'flex',alignItems:'center',gap:12,padding:'12px 14px',background:'var(--cream)',borderRadius:11,cursor:'pointer'}} onClick={() => setShowAddProducts(false)}>
                <div style={{flex:1}}>
                  <div style={{fontSize:12,fontWeight:600,marginBottom:2}}>{a.label}</div>
        ...
        <div className="sheet-section">
          ...
          <div style={{display:'flex',alignItems:'center',gap:6,padding:'7px 12px',background:'var(--cream)',borderRadius:9,marginBottom:12}}>
      ...
      <Sheet open={showSourceFilter} onClose={() => setShowSourceFilter(false)}
        ...
        <div className="sheet-section" style={{display:'flex',flexDirection:'column',gap:9}}>
          {['Any source','Hanger shot ready','Flat-lay ready','Mannequin ready','No source image · needs upload'].map(o => (
            <div key={o} style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'12px 14px',background:'var(--cream)',borderRadius:11,cursor:'pointer'}}>
  ...
  return (
    <div>
      ...
      <Sheet open={showAddModel} onClose={() => setShowAddModel(false)}
        ...
        {/* LIVE PREVIEW HERO */}
        <div style={{display:'flex',gap:14,padding:'0 0 18px',borderBottom:'1px solid var(--mist)',marginBottom:18}}>
          <div style={{position:'relative',width:120,height:150,borderRadius:12,background:`url('${heroPhotoFor(newModelSkin)}') center/cover`,flexShrink:0,overflow:'hidden'}}>
            <div style={{position:'absolute',top:8,left:8,fontSize:7,fontWeight:700,letterSpacing:'0.8px',background:'rgba(26,18,9,0.75)',color:'var(--cream)',padding:'3px 6px',borderRadius:5}}>REPRESENTATIVE PREVIEW</div>
          </div>
          <div style={{flex:1,display:'flex',flexDirection:'column',justifyContent:'space-between'}}>
            <div>
        ...
        <div className="sheet-section">
          ...
          <div style={{position:'relative',border:'1.5px dashed rgba(184,149,90,0.4)',borderRadius:12,padding:'24px 18px',textAlign:'center',cursor:'pointer',background:'rgba(184,149,90,0.03)'}}>
            <div style={{width:38,height:38,borderRadius:10,background:'var(--gold)',display:'flex',alignItems:'center',justifyContent:'center',margin:'0 auto'}}>
          ...
          <div style={{fontSize:9.5,color:'var(--stone)',lineHeight:1.55,marginTop:10,padding:'8px 11px',background:'rgba(184,149,90,0.06)',borderLeft:'2px solid var(--gold)',borderRadius:'0 6px 6px 0',textAlign:'left',fontStyle:'italic'}}>
            ...
            <div style={{display:'flex',flexWrap:'wrap',gap:6}}>
              {group.opts.map(v => (
                <div key={v} onClick={() => group.set(v)}
                  style={{padding:'5px 11px',borderRadius:20,border:`1.5px solid ${group.val===v?'var(--gold)':'var(--mist)'}`,background:group.val===v?'rgba(184,149,90,0.08)':'var(--white)',fontSize:10.5,fontWeight:600,cursor:'pointer'}}>
                  {v}
                </div>
              ))}
            </div>
        ...
        <div className="sheet-section">
          ...
          <input value={newModelName} onChange={e=>setNewModelName(e.target.value)}
            style={{width:'100%',padding:'10px 13px',border:'1.5px solid var(--mist)',borderRadius:10,fontSize:11.5,fontFamily:'inherit',outline:'none',boxSizing:'border-box'}}
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
            {LOOKS.map(l => (
              <div key={l.id} onClick={() => setNewLookBase(l.id)}
                style={{borderRadius:12,border:`1.5px solid ${newLookBase===l.id?'var(--gold)':'var(--mist)'}`,background:'var(--white)',cursor:'pointer',overflow:'hidden'}}>
      ...
      <Sheet open={showBrandMenu} onClose={() => setShowBrandMenu(false)}
        ...
        <div style={{display:'flex',flexDirection:'column',gap:8}}>
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'14px 16px',background:'var(--cream)',borderRadius:11,cursor:'pointer'}}
      ...
      <Sheet open={showNetworkTrends} onClose={() => setShowNetworkTrends(false)}
        ...
        <div className="sheet-section">
          ...
          {NETWORK_TREND_LOOKS.map(t => (
            <div key={t.name} style={{display:'flex',alignItems:'center',gap:10,marginBottom:8}}>
              ...
              <div style={{flex:2,height:8,background:'var(--cream)',borderRadius:4,overflow:'hidden'}}>
                <div style={{height:'100%',background:'var(--gold)',width:`${t.pct*2}%`,borderRadius:4}} />
              </div>
        ...
        <div className="sheet-section">
          ...
          {NETWORK_TREND_RATIOS.map(t => (
            <div key={t.name} style={{display:'flex',alignItems:'center',gap:10,marginBottom:8}}>
              <div style={{flex:1,fontSize:11,fontWeight:600}}>{t.name}</div>
              <div style={{flex:2,height:8,background:'var(--cream)',borderRadius:4,overflow:'hidden'}}>
                <div style={{height:'100%',background:'var(--gold)',width:`${t.pct}%`,borderRadius:4}} />
              </div>
              <div style={{width:36,fontSize:10.5,fontWeight:700,textAlign:'right'}}>{t.pct}%</div>
            </div>
          ))}
        ...
        <div className="sheet-section">
          ...
          <div style={{padding:'14px 16px',background:'rgba(184,149,90,0.06)',borderRadius:11,fontSize:11,lineHeight:1.55}}>
        ...
        <div className="explain-overlay" onClick={() => setShowExplain(false)}>
          <div className="explain-modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="explain-body">
              ...
              <div style={{marginTop:18,padding:'14px 16px',background:'rgba(184,149,90,0.06)',borderRadius:11,fontSize:11,lineHeight:1.55,color:'var(--stone)'}}>
